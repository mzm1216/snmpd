...
#if 0
    int msqid = -1;
    msqid = msgget((key_t)SNMP_MSGKEY, IPC_EXCL);  /*ºÏ≤Èœ˚œ¢∂”¡– «∑Ò¥Ê‘⁄*/  
    if(msqid < 0)
    {
        msqid = msgget((key_t)SNMP_MSGKEY, IPC_CREAT|0666);/*¥¥Ω®œ˚œ¢∂”¡–*/  
        if(msqid <0)
        {  
...
int send_snmpMsg(QUEUE_MSG_T *p_msg)
{
	...
	msqid = msgget((key_t)APP_MSGKEY, IPC_EXCL);/*ºÏ≤Èœ˚œ¢∂”¡– «∑Ò¥Ê‘⁄ */ 
	if(msqid < 0)
	{
	    snmp_log(LOG_DEBUG,"%s:%d  fail to get msg queue\r\n", __FUNCTION__, __LINE__);
		return MSG_GENERROR;
	}
	else
	{
...
int clean_RecvMsgQueue(QUEUE_MSG_T *p_msg)
{
    int msqid = -1; 
...
int recv_snmpMsg(QUEUE_MSG_T *p_msg)
{
    int msqid = -1; 
    int ret_value = MSG_GENERROR;
	int count = 0;
...
{
    int ret_value = MSG_GENERROR;
	unsigned long   oper_code = 0;        /*≤Ÿ◊˜¬Î*/
	QUEUE_MSG_T msg = {0};

    //pthread_mutex_lock(&mut);
	
    /*clean the recv msg queue first*/
	clean_RecvMsgQueue(&msg);
